...
class PitchfxDbManager
    ...
    def find_or_create_rosters(game, away_id, home_id)
      ...
      if home_res.num_rows > 0
        ...
        home_roster_id = insert_roster(game_id, home_id, active_date, 'h')
      end
    ...
    def find_or_create_linescore(linescore, game)
      ...
      linescore.innings.each do |ls_inning|
        ...
        if res.num_rows > 0
          ...
          insert_linescore(game_id, inning_num, ls_inning[0], ls_inning[1])
        end        
        inning_num += 1
      end
    end
    ...
    def update_rosters(game, away_id, home_id)
      ...
      #puts '############ away_roster_id = ' + away_roster_id
    ...
    def insert_game_info(game, game_id)
      ...
      home_text = @db.escape_string("#{boxscore.home_batting_text}")
      away_text = @db.escape_string("#{boxscore.away_batting_text}")
      info_text = @db.escape_string("#{boxscore.game_info}")
    ...
    def insert_batting_stat(batter, game_id, game_date, batter_id)
      name = @db.escape_string("#{batter.batter_name}")
    ...
    def insert_highlight(highlight, game_id)
      headline = name = @db.escape_string(highlight.headline)
    ...
    def add_player_data(lahman_dbname)
      @lahman_db = Mysql.real_connect(@host, @user, @password, lahman_dbname)
      ...
      if res.num_rows > 0
        res.each do |row|
          player_id = row[0]
          first = @db.escape_string(row[1])
          last = @db.escape_string(row[2])
          ...
          if lres.num_rows > 0
            ...
            lres.each do |row|
              ...
              birthDay = row[3]
              if row[4]
                birthCountry = @db.escape_string(row[4])
              end
              debut = row[5]
              if row[6]
                college = @db.escape_string(row[6])
              end
            end
